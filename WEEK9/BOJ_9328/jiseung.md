...
> 현재 가지고 있는 열쇠를 저장하는 배열과\
> 획득하지 못한 열쇠별로 잠겨있어서 이동하지 못하는 위치를 저장하는 리스트를 두어서 bfs를 돌았다.\
> 이미 방문한 위치는 다시 방문하지 않기 위해 노력했다.\
> 열쇠가 있는 칸에 방문할 때도 이미 획득한 열쇠면 해당 위치만 큐에 넣고 넘어가는 방식으로 구현했으면 중복되는 부분을 더 줄일 수 있을 것 같다.

# **📑Related Issues**

> 예전에 풀어본 문제지만 풀 수 있어서 좋았다
